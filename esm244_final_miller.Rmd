...
#  st_transform(crs = wgs84) # Double check crs for WGS84 UTM ZOne 10S datum that transect block points are in. Or just transform them all. I'm pretty sure they are all transformed.

# Simplfy lookup_blocks so only transect block, and UTM are shown. Might need to convert UTM to lat/long
block_gps <- lookup_blocks %>% 
  select(x_proj, y_proj) %>% 
  filter(x_proj != "NA")

#rownames(block_gps) <- lookup_blocks$transect_block
