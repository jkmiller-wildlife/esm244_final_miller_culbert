...
```{r date_time}
vafb_birds_combine$survey_date <- mdy(vafb_birds_combine$survey_date)
...
vafb_birds_combine %>% 
  ...
  ggplot() +
  geom_point(aes(x = survey_date, y = count, color = beach_section_initials)) +
...
birds_by_sector <- vafb_birds_combine %>% 
  group_by(survey_date, beach_section_initials, species, species_type) %>% 
  summarize(total = sum(count))

birds_by_region <- vafb_birds_combine %>% 
  group_by(survey_date, beach, species, species_type) %>% 
  summarize(total = sum(count))
...
birds_by_region %>% 
  ...
  filter(beach == "PURISIMA") %>% 
  ggplot() +
  geom_point(aes(x = survey_date, y = total)) +
  geom_line(aes(x = survey_date, y = total)) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
...
#wrack_simple$survey_date <- mdy(wrack_simple$survey_date) # This line of code just messes things up.
...
wrack_beach <- full_join(wrack_simple, lookup_blocks, by = "transect_block") %>% 
  select(survey_date, transect_block, wrack, beach_section_initials, beach) %>% 
  filter(beach == "NORTH" | beach == "SOUTH" | beach == "PURISIMA")
...
wrack_mean_sector <- wrack_beach %>% 
  group_by(survey_date, beach_section_initials) %>% 
...
# Widget 1: Dropdown/radio button selects species or species type. 
# Widget 2: Slider scale selects date. 
# Widget 3: Dropdown/radio button selects beach sector (MIN, SHN, SHN, etc.) or region (North/Purisima/South). Might be more informative to just keep at at a North/Purisima vs. South Option.
...
ggplot(wrack_beach) +
  geom_line(aes(x = survey_date, y = wrack))
...
# Result: Graph of species or species type population time series. Limit this one to SNPL, SAND, and gulls. Grouped by Beach Sector at smallest.
