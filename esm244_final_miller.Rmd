...
blocks <- lookup_blocks %>% 
  dplyr::select(transect_block, beach_section_initials, beach)
...
avian_clean <- avian_block_join %>% 
  dplyr::select(survey_week, transect_block, beach_section_initials, beach, species, species_name, species_type, count) %>% 
  filter(beach == "NORTH" | beach == "PURISIMA" | beach == "SOUTH") %>% 
  filter(species != "NA")
...
birds_by_sector <- vafb_birds_combine %>% 
  group_by(survey_week, beach_section_initials, species, species_type) %>% 
  summarize(total = sum(count))

birds_by_region <- vafb_birds_combine %>% 
  group_by(survey_week, beach, species, species_type) %>% 
  summarize(total = sum(count))
...
ggplot(wrack_beach) +
  geom_line(aes(x = survey_week, y = wrack))
